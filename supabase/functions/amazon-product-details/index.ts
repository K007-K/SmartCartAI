...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`Fetching product details for ASIN: ${asin}, Country: ${country}`);
    ...
    // Build the URL with query parameters, exactly matching the Python example
    const queryParams = {
      asin: asin,
      country: country
    };
    ...
    try {
      ...
      // Format the product details to match what the frontend expects
      ...
      console.log("Successfully extracted product details:", JSON.stringify({
        asin: productDetails.asin,
        title: productDetails.title.substring(0, 30) + "..."
      }));
    ...
    console.error("Error in amazon-product-details function:", error);
    return new Response(
      JSON.stringify({ error: error.message || "An unexpected error occurred", status: "ERROR" }),
      { 
        status: 500, 
        headers: { ...corsHeaders, "Content-Type": "application/json" } 
      }
